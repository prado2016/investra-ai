...
/**
 * Modified Mock Data Insertion Script
 * This version prompts for user ID to bypass authentication issues
 */
...
const DEFAULT_USER_ID = '1845c30a-4f89-49bb-aeb9-bc292752e07a'
...
const generateMockTransactions = (portfolioId, assets) => {
  ...
  assets.forEach((asset, index) => {
    ...
    transactions.push({
      ...
      fees: 9.99,
      transaction_date: generateDateInPast(20 - index),
    ...
    if (index < 2) { // Only AAPL and GOOGL pay dividends in our mock
      transactions.push({
        ...
        fees: 0,
        transaction_date: generateDateInPast(3 - index),
...
async function insertMockData() {
  ...
  try {
    // Ask for user ID
    console.log(`📝 I found this user ID in your logs: ${DEFAULT_USER_ID}`)
    const useDefault = await askQuestion('Use this user ID? (y/n): ')
    ...
    console.log('2. Set VITE_USE_MOCK_DASHBOARD=false in your .env file')
    console.log('3. Restart your dev server to see real data')
